...
public class OutboxProcessor {
  ...
  /**
   * Given a group of outbox items, process them in order. Items are passed to the handle provider in order, from the
   * oldest to the youngest. If an item cannot be processed right now, it stops the group processing altogether. If an
   * item fails to be processed, it stops the group processing.
   *
  ...
  /**
   * Process an item. Return the state of the processing. A temporary error won't prevent the item from being process
   * again later, a definitive error put the item in the DLQ (where it won't be processed again).
   *
  ...
  /**
   * Mark an item as failed. If an item failed more than the configured amount of times, it is put in the DLQ and never
   * processed again.
   *
